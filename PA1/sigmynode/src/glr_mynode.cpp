...
void GlrMyNode::render ( SnShape* s, GlContext* ctx )
{
	...
	//Vectors for the triangles which create the rectangle.
	GsVec P1, P2, P3, P4;
	...
	{	
		...
		GsArray<GsVec> P(0, (int)(c.theta/c.n)*4); // will hold the points forming my triangles (size 0, but pre-allocate enough spaces)

		for (double theta1 = 0.0f; theta1 < c.theta; theta1 += c.theta/c.n) {

			for (double phi2 = 0.0f; phi2 < c.phi; phi2 += c.phi/c.n) {

				//Using the equation given from the guidelines.
				P1.set(
					(c.R + c.r * cos(theta1)) * cos(phi2),
					(c.R + c.r * cos(theta1)) * sin(phi2),
					c.r * sin(theta1)
						);
				
				 P2.set(
					 (c.R + c.r * cos(theta1 + (c.theta / c.n))) * cos(phi2),
					 (c.R + c.r * cos(theta1 + (c.theta / c.n))) * sin(phi2), 
					 c.r * sin(theta1 + (c.theta / c.n))
						);

				 P3.set(
					 (c.R + c.r * cos(theta1 + (c.theta / c.n))) * cos(phi2 + (c.phi / c.n)),
					 (c.R + c.r * cos(theta1 + (c.theta / c.n))) * sin(phi2 + (c.phi / c.n)), 
					 c.r * sin(theta1 + (c.theta / c.n))
						);

				 P4.set(
					 (c.R + c.r * cos(theta1)) * cos(phi2 + (c.phi / c.n)),  //X
					 (c.R + c.r * cos(theta1)) * sin(phi2 + (c.phi / c.n)),	//Y
					 c.r * sin(theta1)	//Z
						);

				//Pushing the points counter clockwise.
				P.push() = P1;
				P.push() = P2;
				P.push() = P3;
				//2nd Triangle
